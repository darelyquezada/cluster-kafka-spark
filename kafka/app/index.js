...
async function run() {
  try {
    ...
    console.log(`Iniciando el envío por LOTES de ${datosMasivos.length} registros...`);
    
    const TAMANO_LOTE = 1000; // Enviamos 1000 mensajes de golpe
    
    for (let i = 0; i < datosMasivos.length; i += TAMANO_LOTE) {
      const lote = datosMasivos.slice(i, i + TAMANO_LOTE);
      
      // Mapeamos los datos al formato que pide Kafka
      const mensajesKafka = lote.map(registro => ({
        key: `key-${registro.id_persona}`,
        value: JSON.stringify(registro)
      }));

      // Elegimos un tópico aleatorio para este lote (para simplificar)
      const topicDestino = topicos[Math.floor(Math.random() * topicos.length)];

      // Enviamos el lote entero en una sola petición de red
      await producer.send({
        topic: topicDestino,
        messages: mensajesKafka,
      });

      console.log(`[PRODUCTOR] Se han enviado ${Math.min(i + TAMANO_LOTE, datosMasivos.length)} mensajes al clúster...`);
    }
